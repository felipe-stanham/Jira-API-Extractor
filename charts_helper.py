"""Helper functions for creating charts in Excel."""

from openpyxl.chart import PieChart, BarChart, Reference
from openpyxl.chart.label import DataLabelList

def create_enhanced_charts_sheet(wb, issues, worklogs=None):
    """Creates a comprehensive charts sheet with multiple visualizations."""
    ...
    if 'Sprint Issues' in wb.sheetnames:
        ...
        for i, status in enumerate(sorted(unique_statuses)):
            ...
            # Use COUNTIF formula to count occurrences
            formula = f'=COUNTIF(\'Sprint Issues\'.D:D,"{status}")'
    ...
    if 'Sprint Issues' in wb.sheetnames:
        ...
        for i, issue_type in enumerate(sorted(unique_types)):
            ...
            # Use COUNTIF formula to count occurrences
            formula = f'=COUNTIF(\'Sprint Issues\'.B:B,"{issue_type}")'
            ws_charts.cell(row=row, column=2, value=formula)
    ...
    if worklogs and 'Work Logs' in wb.sheetnames:
        ...
        for i, issue_type in enumerate(sorted(unique_worklog_types)):
            ...
            ws_charts.cell(row=row, column=1, value=issue_type)
            # Use SUMIF formula to sum hours by issue type
            formula = f'=SUMIF(\'Work Logs\'.B:B,"{issue_type}",\'Work Logs\'.G:G)'
            ws_charts.cell(row=row, column=2, value=formula)
        ...
        for i, author in enumerate(unique_authors):
            ...
            col = 2
            for issue_type in unique_worklog_types:
                # SUMIFS formula to sum hours by author and issue type
                formula = f'=SUMIFS(\'Work Logs\'.G:G,\'Work Logs\'.E:E,"{author}",\'Work Logs\'.B:B,"{issue_type}")'
                ws_charts.cell(row=row, column=col, value=formula)
                col += 1
            
            # Total for author
            total_formula = f'=SUMIF(\'Work Logs\'.E:E,"{author}",\'Work Logs\'.G:G)'
