...
2. If both are null → use 0
3. If one is not null → use that value
4. If both are not null → use "Story Points" field (priority)

### Solution 2: JQL Filter Parameter
**Scope**:
- ✅ Applies to: Sprint issues, Open epic issues
- ❌ Does NOT apply to: Comments sheet, Worklogs sheet
...
- [ ] Test JQL filtering with sprint issues (add JQL to query to bring only issue with status "In Progress")
- [ ] Test JQL filtering with epic issues (add JQL to query to bring only issue with status "In Progress")
- [ ] Test that JQL does NOT filter comments/worklogs (add JQL to query to bring only issue with status "In Progress")
...
### Story Points
✅ Extracts from "Story Points" field when available
✅ Falls back to "Story Points Estimate" when "Story Points" is null
✅ Returns 0 when both fields are null
✅ Prioritizes "Story Points" when both fields have values
✅ Works across all sheets (Sprint, Epic Label, Open Epics, Progress)

### JQL Filter
✅ JQL parameter available in command line
✅ JQL parameter available in Streamlit UI
✅ JQL correctly filters sprint issues
✅ JQL correctly filters epic issues
✅ JQL does NOT filter comments
✅ JQL does NOT filter worklogs
✅ JQL properly combines with existing queries (AND logic)
✅ Empty/null JQL works (no filtering applied)

### Documentation
✅ README updated with JQL usage examples
